				...
				if (submit.equals("fail")) {
					...
					try {
						...
						if (jsonObject.getBoolean("success")) {
							Builder builder = new Builder(OnlineActivity.this);
							AlertDialog aDialog = builder.setIcon(R.drawable.success).setTitle("提交成功")
									.setCancelable(false)
		...
		findViewById(R.id.gallery).setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				if ((intent.resolveActivity(getPackageManager()) != null)) {
					...
					Intent intentTemp = new Intent(OnlineActivity.this, GalleryActivity.class);
		...
		findViewById(R.id.submit).setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				if (wenTiContent == null || wenTiContent.equals("")) {
					Toast.makeText(getApplicationContext(), "请输入您需要咨询的问题。", Toast.LENGTH_SHORT).show();
				} else if (files.size() < 1) {
					Toast.makeText(getApplicationContext(), "请上传至少一张图片", Toast.LENGTH_SHORT).show();
				} else {
					progress = ProgressDialog.show(OnlineActivity.this, null, "正在提交...");
					params.put("MiaoShu", wenTiContent);
					new Thread() {
						public void run() {
							submit = OKHttp.uploadFiles(UrlString.LiuPeng + "/ZhiXunWenTiAction", params, files);
